...
parser.add_argument('--edge_label_file', type=str, required=True, help='Path to the edge labels pickle file')
parser.add_argument('--path_label_file', type=str, required=True, help='Path to the path labels pickle file')
parser.add_argument('--dataset_name', type=str, default='magazine_graph', help='Name of the dataset')
parser.add_argument('--valid_ratio', type=float, default=0.1) 
parser.add_argument('--test_ratio', type=float, default=0.2)
parser.add_argument('--max_num_samples', type=int, default=-1, help='maximum number of samples to explain, for fast testing. Use all if -1')
...
parser.add_argument('--prune_max_degree', type=int, default=200, help='prune the graph such that all nodes have degree smaller than max_degree. No prune if -1') 
parser.add_argument('--save_explanation', default=False, action='store_true', help='Whether to save the explanation')
parser.add_argument('--saved_explanation_dir', type=str, default='saved_explanations', help='directory of saved explanations')
...
g, processed_g, pred_pair_to_edge_labels, pred_pair_to_path_labels = load_amazon_fashion_dataset(
    args.review_file, args.item_file, args.edge_label_file, args.path_label_file, args.valid_ratio, args.test_ratio)
...
pagelink = PaGELink(model, lr=args.lr, alpha=args.alpha, beta=args.beta, num_epochs=args.num_epochs, log=True).to(device)
...
for i in tqdm(test_ids[:10]):
    ...
    if pred:
        ...
        paths, comp_g_edge_mask_dict = pagelink.explain(src_nid, tgt_nid, mp_g, args.num_hops, args.prune_max_degree, args.k_core, args.num_paths, args.max_path_length, return_mask=True)
